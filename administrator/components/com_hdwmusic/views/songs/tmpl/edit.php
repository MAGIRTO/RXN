...
<div id="hdwmusic">
  <p class="hdwheader"><?php echo JText::_('EDIT_SONG'); ?></p>
  <form action="index.php?option=com_hdwmusic&task=songs" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
    <table class="admintable">
      <tr>
        ...
        <td><input type="text" name="song_title" size="60" value="<?php echo $data->song_title; ?>" ></td>
      </tr>
      <tr>
        <td class="hdwkey"></td>
        <td class="hdwkey"><input type="radio" name="song_type" value="upload" <?php if($data->song_type == 'upload') echo 'checked="checked"'; ?> onclick="changeType('upload')" />
          <?php echo JText::_('UPLOAD'); ?>&nbsp;&nbsp;
          <input type="radio" name="song_type" value="url" <?php if($data->song_type == 'url') echo 'checked="checked"'; ?> onclick="changeType('url')" />
          <?php echo JText::_('URL'); ?></td>
      </tr>
      <tr id="upload_music_data">
        <td class="hdwkey"><?php echo JText::_('UPLOAD_MUSIC_FILE'); ?></td>
        <td id="song_upload_file"><?php if($data->song_file) { ?>
          <input name="song_upload_file" readonly="readonly" value="<?php echo $data->song_file; ?>" size="47" />
          <input type="button" value="Change" onclick="changeMode('file')" />
          <?php } else { ?>
          <input type="file" name="song_upload_file" maxlength="100" />
          <?php } ?>
        </td>
      </tr>
      <tr id="upload_thumb_data">
        <td class="hdwkey"><?php echo JText::_('UPLOAD_THUMB_IMAGE'); ?></td>
        <td id="song_upload_thumb"><?php if($data->song_thumb) { ?>
          <input name="song_upload_thumb" readonly="readonly" value="<?php echo $data->song_thumb; ?>" size="47" />
          <input type="button" value="Change" onclick="changeMode('thumb')" />
          <?php } else { ?>
          <input type="file" name="song_upload_thumb" maxlength="100" />
          <?php } ?>
        </td>
      </tr>
      <tr id="music_url_data">
        <td class="hdwkey"><?php echo JText::_('MUSIC_FILE_PATH'); ?></td>
        <td><input type="text" name="song_file" size="60" value="<?php echo $data->song_file; ?>" /></td>
      </tr>
      <tr id="thumb_url_data">
        <td class="hdwkey"><?php echo JText::_('THUMB_IMAGE'); ?></td>
        <td><input type="text" name="song_thumb" size="60" value="<?php echo $data->song_thumb; ?>" /></td>
      </tr>
      <tr>
        ...
        <td><select id="song_album" name="song_album">
          ...
          <?php echo '<script>document.getElementById("'.$data->song_album.'").selected="selected"</script>'; ?>
        </td>
      </tr>
      <tr>
        <td class="hdwkey" valign="top" style="padding-top:10px;"><?php echo JText::_('SONG_LYRICS'); ?></td>
        <td><?php echo $editor->display('song_lyrics', $data->song_lyrics, '700', '400', '60', '20', true); ?></td>
      </tr>
      <tr>
...
var isAllowed       = true;
changeType('<?php echo $data->song_type; ?>'); 
...
if(<?php echo substr(JVERSION,0,3); ?> == '3.0') {
 ...
 function changeMode(inp) {	
    var mode;
    mode='<input type="file" name="song_upload_' + inp + '" maxlength="100" />';
	document.getElementById("song_upload_" + inp).innerHTML = mode;
}
