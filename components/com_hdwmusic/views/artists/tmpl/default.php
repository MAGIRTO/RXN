...
$link      = 'index.php?option=com_hdwmusic&view=artist&id=';
$row       = 0;
$column    = 0;
...
<div id="hdw_letterbar<?php echo $this->escape($this->params->get('pageclass_sfx')); ?>" <?php if (JRequest::getVar('show_letter_bar') == 0) echo 'style="display:none;"'; ?> >
  ...
  ?>
  <a href="<?php echo JRoute::_('index.php?option=com_hdwmusic&view=artist&char='.$char.$qs); ?>" <?php echo $style; ?>><?php echo $chars[$i]; ?></a>
  <?php } ?>
</div>
<div id="hdw_gallery<?php echo $this->escape($this->params->get('pageclass_sfx')); ?>">
  <?php 
    if(!count($artists)) echo JText::_('ITEM_NOT_FOUND');
  	for ($i=0, $n=count($artists); $i < $n; $i++) {  
		...
		echo $clear;
  ?>
  <div class="hdw_thumb"> <a href="<?php echo JRoute::_($link.$artists[$i]->artist_slug.$qs); ?>">
    <h3><?php echo $artists[$i]->artist_name; ?></h3>
    <img src="<?php echo $artists[$i]->artist_thumb; ?>" width="218" height="150" title="<?php echo 'Click to View : '.$artists[$i]->artist_name; ?>" border="0" onmouseover="this.style.opacity=0.7" onmouseout="this.style.opacity=1"/> <span class="views"><strong>views : </strong><?php echo $artists[$i]->artist_views; ?></span> </a> </div>
  <?php } ?>
</div>
